...
class Settings(BaseSettings):
    ...
    system_prompt: str = Field(
        default=(
            ...
            "ultime N tracce atterrate col fallback. Il tool può tornare con "
            "`status: \"needs_clarification\"` + un campo `question` + `candidates`: "
            "in quel caso NON improvvisare, ma RIPORTA LETTERALMENTE la `question` "
            "all'utente (puoi riformularla in italiano naturale, con le opzioni da "
            "`candidates` se presenti). Appena l'utente risponde, richiama "
            "`music_retag_track(job_id=..., hint=<risposta utente>)` con la risposta "
            "passata come `hint`. Ripeti il ciclo finché il tool ritorna "
            "`status: \"placed\"`. Non scrivere metadati inventati, non saltare il "
